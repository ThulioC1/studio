...
import { Building2, MapPin, Users, Activity, Phone, Printer, Mail } from 'lucide-react';
...
export function CompanyDetails({ company }: CompanyDetailsProps) {
  ...
  const razaoSocial = company?.company?.name || 'Razão Social não informada';
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 bg-card rounded-2xl border shadow-sm print:border-none print:shadow-none print:p-0">
        <div className="space-y-1">
          <div className="flex items-center gap-3 flex-wrap">
            <h2 className="text-2xl font-bold text-primary">{razaoSocial}</h2>
          ...
          <TabsContent value="geral" className="space-y-4">
            <Card>
              <CardContent className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                <DataField label="Razão Social" value={razaoSocial} />
                ...
                <DataField label="Natureza Jurídica" value={company?.company?.nature?.text} />
                <DataField label="Porte" value={company?.company?.size?.text} />
                <DataField label="Capital Social" value={formatCurrency(company?.company?.equity || 0)} />
          ...
          <TabsContent value="socios" className="space-y-4">
            <Card>
              <CardContent className="p-6">
                {company?.company?.members && company.company.members.length > 0 ? (
                  <div className="space-y-4">
                    {company.company.members.map((socio, i) => (
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="border p-4 rounded-lg">
            ...
            <div className="space-y-1 text-sm">
              ...
              <p><strong>Capital Social:</strong> {formatCurrency(company?.company?.equity || 0)}</p>
            </div>
          </div>
          <div className="border p-4 rounded-lg">
          ...
          {company?.company?.members && company.company.members.length > 0 && (
            <div className="border p-4 rounded-lg col-span-2">
              ...
              <ul className="grid grid-cols-2 gap-2 mt-2">
                {company.company.members.map((s, i) => (
